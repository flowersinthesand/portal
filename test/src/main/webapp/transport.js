		...
		return {
					...
					send: function(event, data, callback) {
						setTimeout(function() {
							if (accepted) {
								...
								socket._fire(portal.support.isBinary(data) ? data : portal.support.stringifyJSON({
									id: eventId, 
									reply: !!callback,
									type: event, 
									data: data
			...
			send: function(data) {
				setTimeout(function() {
					if (accepted) {
						var latch,
							event = portal.support.isBinary(data) ? {type: "message", data: data} : portal.support.parseJSON(data),
							args = [event.data];
						...
						connection.trigger(event.type, args);
					}
				}, 5);
			},
			close: function() {
